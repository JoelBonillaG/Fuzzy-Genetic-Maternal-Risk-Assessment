...
def main():
    ...
    candidatos = cargar_candidatos(entrada, args.algoritmo, args.iteracion)
    if not candidatos:
    ...
    guardar_json(Path(args.salida_metadata), construir_metadata(seleccionado))
...
def crear_parser():
    ...
    parser.add_argument("--algoritmo", default="GENETIC_ALGORITHM")
    parser.add_argument("--iteracion", type=int, default=None)
...
def cargar_candidatos(entrada: Path, algoritmo: str, iteracion: int | None):
    ...
    for ruta in rutas:
        ...
        if not isinstance(contenido, dict) or "reglas_finales" not in contenido:
            continue
        algoritmo_candidato = normalizar_algoritmo(contenido.get("algoritmo") or inferir_algoritmo(ruta))
        if algoritmo and algoritmo_candidato != normalizar_algoritmo(algoritmo):
            continue
...
def inferir_algoritmo(ruta: Path):
    nombres = {
        "genetic_algorithm": "GENETIC_ALGORITHM",
        "ripper": "RIPPER",
        "prism": "PRISM",
    }
    return nombres.get(ruta.stem, ruta.stem)
...
def preparar_candidato(candidato: dict, datos: dict, membresias: dict):
    ...
    return {
        ...
        "algoritmo": normalizar_algoritmo(contenido.get("algoritmo") or inferir_algoritmo(candidato["ruta"])),
...
def construir_metadata(candidato: dict):
    return {
        "iteracion": candidato["iteracion"],
        "metricas": sin_matriz(candidato["metricas_limpias"]),
        "resumen_reglas": resumir_reglas(
            candidato["reglas_comunes"],
            candidato["duplicadas_eliminadas"],
        ),
        "matriz_confusion": candidato["metricas_limpias"]["matriz_confusion"],
    }
