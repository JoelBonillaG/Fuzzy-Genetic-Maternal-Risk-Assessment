"""Comparacion experimental de RIPPER, PRISM y algoritmo genetico.

El experimento usa el dataset completo, sin particiones train/test.
...
from ..optimizacion.genetic_algorithm import (
    contar_duplicados,
...
def ejecutar_experimento(config):
    ...
    ejecutar_limpieza_automatica(ruta_resultados)
    return resumen
...
def ejecutar_limpieza_automatica(ruta_resultados):
    from .limpiar_reglas_iteraciones import limpiar_resultados

    print("\nLimpiando reglas duplicadas y recalculando metricas...")
    resumen = limpiar_resultados(entrada=ruta_resultados)
    mejor_ag = resumen.get("mejor_ag")
    if mejor_ag:
        print(
            "Mejor AG sin duplicadas | "
            f"iteracion={int(mejor_ag['iteracion']):02d} | "
            f"BA={mejor_ag['ba_limpia']:.4f} | "
            f"Accuracy={mejor_ag['accuracy_limpia']:.4f} | "
            f"reglas={int(mejor_ag['reglas_limpias'])}"
        )
...
def ejecutar_ag(iteracion, tabla, ruta_iteracion, config):
    print("  AG     | evolucionando poblacion de reglas...")
    mejor, historial = ejecutar_algoritmo_genetico(
        tabla=tabla,
        membresias=construir_membresias_base(),
        parametros=config["genetic_algorithm"],
    )
    reglas = mejor.reglas
    asignar_origen(reglas, "GENETIC_ALGORITHM")
    resultado = evaluar_y_guardar(
        iteracion=iteracion,
